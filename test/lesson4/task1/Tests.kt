...
class Tests {
    ...
    fun russian() {
        ...
        assertEquals("девятьсот тридцать семь тысяч семьсот семьдесят четыре", russian(937774))
        assertEquals("двенадцать", russian(12))
        assertEquals("двести тридцать пять тысяч пятьсот десять", russian(235510))
    }
}